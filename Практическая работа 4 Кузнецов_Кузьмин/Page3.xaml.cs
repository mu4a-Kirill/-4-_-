...
namespace Практическая_работа_4_Кузнецов_Кузьмин
{
    public partial class Page3 : Page
    {
        ...
        public double CalculateY(double x, double d)
        {
            if (Math.Abs(x) < 1e-9)
                throw new DivideByZeroException("x не может быть равен нулю.");

            double argument = 5 * x + d / x;
            double tanValue = Math.Tan(argument);

            if (double.IsInfinity(tanValue) || double.IsNaN(tanValue))
                throw new InvalidOperationException("Тангенс не определён для данного аргумента.");

            double y = x * x + tanValue;

            if (Math.Abs(y) > 1e6)
                throw new OverflowException("Значение y слишком велико.");
        ...
        private void UpdateGraph()
        {
            ...
            for (double x = x0; x <= xk; x += dx)
            {
                ...
                {
                    ResultTextBox.AppendText($"x = {x:F3}: {ex.Message}\r\n");
                    hasErrors = true;
                }
            }
