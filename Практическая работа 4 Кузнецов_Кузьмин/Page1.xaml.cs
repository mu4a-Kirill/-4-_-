...
namespace Практическая_работа_4_Кузнецов_Кузьмин
{
    public partial class Page1 : Page
    {
        ...
        public double CalculateBeta(double x, double y, double z)
        {
            if (z < -1 || z > 1)
                throw new ArgumentOutOfRangeException(nameof(z), "Z должно быть в диапазоне [-1, 1] для арксинуса.");
            ...
            if (underSqrt < 0)
                throw new InvalidOperationException("Подкоренное выражение не может быть отрицательным.");
        ...
        {
            if (!TryParseDouble(XTextBox.Text, out double x) || !TryParseDouble(ZTextBox.Text, out double z))
        ...
        {
            if (!TryParseDouble(XTextBox.Text, out double x) || !TryParseDouble(YTextBox.Text, out double y))
